...
<div class="bacic-info mb-3">
    ...
    <div class="col-xl-3">
         ...
         <Select class="form-control mb-3" {{(isset($data->branch_id) ) ?  'disabled':''}} required name="branch_id">
            <option value="">Select Branch</option> 
         @if(!empty($gerBranch))
                @foreach($gerBranch as $row)
                <option value="{{$row->id}}" {{(isset($data->branch_id) && $data->branch_id==$row->id) ?  'selected':''}}>{{$row->name}}</option>
                @endforeach
            @endif
         </select>
            @if ($errors->has('branch_id'))
                <span class="text-danger text-left">{{$errors->first('branch_id')}}</span>
            @endif
        </div>    
    <div class="col-xl-3">
...
<div class="Security">
    <div class="row">
        <div class="col-xl-12">
            <a href="{{route('restaurent.manager')}}" class="btn btn-outline-primary float-end ms-3">Back</a>
